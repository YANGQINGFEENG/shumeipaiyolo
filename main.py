...
def main():
    ...
    print("=" * 60)
    print("  智慧农业传感器与执行器控制")
    print("  Smart Agriculture Sensor & Actuator Control")
    print("=" * 60)
    print()
    ...
    # 注册本地执行器
    actuator_ctrl.register_actuator("relay_001", hub.get("relay"))
    actuator_ctrl.register_actuator("laser_001", hub.get("laser"))
    actuator_ctrl.register_actuator("rgb_led_001", hub.get("rgb_led"))
    ...
    print()
    logger.info(f"开始运行...")
    logger.info(f"  数据上传间隔: {UPLOAD_INTERVAL} 秒")
    logger.info(f"  执行器轮询间隔: {POLL_INTERVAL} 秒")
            ...
            if current_time - last_upload_time >= UPLOAD_INTERVAL:
                ...
                for name, reading in readings.items():
                    if reading["status"] == "ok":
                        data = reading["data"]
                        if name == "dht":
                            logger.info(f"  {name}: T={data.get('temperature', 'N/A')}°C H={data.get('humidity', 'N/A')}%")
                        elif name == "bmp280":
                            logger.info(f"  {name}: T={data.get('temperature', 'N/A')}°C P={data.get('pressure', 'N/A')}hPa")
                        elif name == "vibration":
